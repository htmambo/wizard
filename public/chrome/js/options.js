...
OptionsController.prototype = {
    ...
    saveToFileClick: function () {
        ...
        const fileNameToSaveAs = "wizarder.json";
    ...
    allowExistCheckClick: function (e) {
        if (this.protocolCheck_.checked || (this.data.Url && this.data.Url.indexOf('localhost') > 0)) {
            ...
            api.PasswordToken()
                .then(a => {
                    ...
                    if (!cache.check('allProjects')) {
                        api.GetProjects()
                           .then(data => { cache.set('allProjects', data); });
                    }
                })
                .catch(a => {
                    Object.assign(this.data, api.data);
