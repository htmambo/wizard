...
// 容器
    Class.pt.vessel = function (conType, callback) {
        ...
        callback([
            ...
            // 内容区
            + '<div' + (config.id ? ' id="' + config.id + '"' : '') + ' class="layui-layer-content' + ((config.type == 0 && config.icon !== -1) ? ' layui-layer-padding' : '') + (config.type == 3 ? ' layui-layer-loading-box' : '') + '">'
            // 表情或图标
            + function () {
                var face = [
                    'fa-info-circle',
                ...
                if (config.type == 0 && config.icon !== -1) {
                    // 加载（加载图标）
                    if (config.icon == 16) {
                        additFaceClass = 'fa fa-spinner fa-spin fa-3x fa-fw ' + animClass;
                    } else if (config.icon == 17) {
                        additFaceClass = 'fa fa-spinner fa-pulse fa-3x fa-fw ' + animClass;
                    }
                    return '<i class="layui-layer-face fa ' + (
                ...
                if (config.type == 3) {
                    ...
                    // 风格 2
                    var append = ' fa-spin fa-3x fa-fw ';
                    if (config.icon > 3) {
                        append = ' fa-pulse fa-3x fa-fw ';
                        config.icon = 0;
                    }
                    return '<i class="layui-layer-loading-icon fa ' + (
            ...
            + '<div class="layui-layer-setwin">' + function () {
                ...
                // 关闭按钮
                if (config.closeBtn) {
                    arr.push('<span class="fa fa-close ' + [
                        doms[7],
