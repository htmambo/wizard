...
class Readability implements LoggerAwareInterface
{
    ...
    public function prepArticle(\DOMNode $articleContent): void
    {
        ...
        if (!$this->flagIsActive(self::FLAG_DISABLE_POSTFILTER)) {
            try {
                $content = $articleContent->getInnerHTML();
                foreach ($this->post_filters as $search => $replace) {
                    $content = preg_replace($search, $replace, $content);
                }
                unset($search, $replace);
                $content = preg_replace('!</?span[^>]*>!is', '', $content);
                // 将 $content 中的所有 <pre> ... </pre> 中的所有`<br />`替换为PHP_EOL，并删除空行
                $content = preg_replace_callback('/<pre([^>]*)>(.*?)<\/pre>/is', function($matches) {
                    // 将<br />替换为换行符
                    $preContent = preg_replace('/<br\s*\/?>\n*/i', PHP_EOL, $matches[2]);
                    // 删除连续的空行 可能会破坏原有结构，需要根据实际情况决定是否删除
                    $preContent = preg_replace('/\n{2,}/m', "\n", $preContent);
                    return '<pre' . $matches[1] . '>' . $preContent . '</pre>';
                }, $content);
    ...
    protected function grabArticle(?\DOMElement $page = null)
    {
        ...
        for ($nodeIndex = 0; $allElements->item($nodeIndex); ++$nodeIndex) {
            ...
            if (mb_strlen($unlikelyMatchString) > 3 // don't process "empty" strings
                ...
                // 代码块中的注释需要保留
                if(!strpos($node->getNodePath(), 'pre/code') || $node->getAttribute('class') !== 'token comment') {
