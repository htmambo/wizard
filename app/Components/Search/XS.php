...
class XSSearch extends XSServer
{
	...
	public function highlight($value, $strtr = false)
	{
		...
		$value = preg_replace('@</em><em[^>]*>@i', '', $value);
		$value = preg_replace('@<em[^>]*>([^<]*)<em[^>]*>@i', '<em class="highlighted">\1', $value);
		$value = preg_replace('@</em>([^<]*)</em>@i', '\1</em>', $value);
		return $value;
	}
	...
	private function initHighlight()
	{
		...
		foreach ($terms as $term) {
			if (!preg_match('/[a-zA-Z]/', $term)) {
				$pairs[$term] = '<em class="highlighted">' . $term . '</em>';
			} else {
				$pattern[] = '/' . strtr($term, array('+' => '\\+', '/' => '\\/')) . '/i';
				$replace[] = '<em class="highlighted">$0</em>';
			}
		}
