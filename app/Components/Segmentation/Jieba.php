...
class Jieba
{
    ...
    public static function init($options = array())
    {
        ...
        $xdb = new \App\Components\XTreeDB;
        $xdb->setReversedCfg('iiii', 'ItfTotal/ItfMax/ItfMin/ItotalWords');
        if (!$xdb->Open(__DIR__ . '/dict/jieba.xdb')) {
    ...
    public static function loadUserDict($f_name, $options = array())
    {
        ...
        while (($line = fgets($content)) !== false) {
            ...
            self::$original_freq[$word] = ['tf' => $freq, 'end' => 1];
            self::$FREQ[$word] = $freq;
            self::$total += $freq;
            $len = mb_strlen($word, 'UTF-8');
            for($i=0;$i<$len;$i++) {
                $tmp = mb_substr($word, 0, $i, 'UTF-8');
                if (!isset(self::$original_freq[$tmp])) {
                    self::$original_freq[$tmp] = ['tf' => 0, 'end' => 0];
                } else {
                    self::$original_freq[$tmp]['end'] = 0;
                }
            }
        }
        fclose($content);
    ...
    public static function getDAG($sentence, $options = array())
    {
        ...
        while ($i < $N) {
            ...
            if($wordInfo = self::checkWord($next_word_key)) {
    ...
    public static function checkWord($word) {
        ...
        if(self::$user_dictname) {
            if(isset(self::$original_freq[$word])) {
                $wordInfo = self::$original_freq[$word];
            }
        }
        if(!isset(self::$FREQ[$word])) {
            if($wordInfo === false) {
        ...
        if(self::$user_dictname) {
            self::$original_freq[$word] = $wordInfo;
        }
        return $wordInfo;
    }
